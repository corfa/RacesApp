...
from db.queries.owner import create_owner
...
class OwnerTab(ttk.Frame):
    def __init__(self, master, ses: Session):
        ...
        self.ses = ses
        self.ButtSubmit = Button(self, text="add Owner", command=self.InpInDB)
        ...
        self.ButtUpdate = Button(self, text="update Data", command=self.updateOwner)
    ...
    def updateOwner(self):
        try:
            up_data(self.ses, Owners, self.inpIdForUpdate.get(), self.inpColumnForUpdate.get(),
    ...
    def InpInDB(self):
        ...
        testbodyforOnwer = dict(FirstName=FirstName, LastName=LastName, Patronymic=Patronymic, Age=Age,
                                PhoneNumber=PhoneNumber)
        try:
            create_owner(self.ses, testbodyforOnwer)
            self.ses.commit()
        except:
